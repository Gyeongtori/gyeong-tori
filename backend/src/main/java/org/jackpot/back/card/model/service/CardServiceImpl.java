...
public class CardServiceImpl implements CardService{
    ...
    @Override
    public List<ReadCardResponse> getCardList(User user,Language language) {
        try {
            ...
            if(language==Language.EN){
                ...
                for(CardRedisEN cardRedis : cardRedisList){
                    if(response.isEmpty()){
                        ...
                        //보유 카드 조회
                        List<HoldingCardEN> holdingCardList = holdingCardENRepository.findByUserIdAndCardNumber(user.getId(), cardRedis.getNumber());
                        List<LocalDate> holdingCards = holdingCardENRepository.findDatesByUserIdAndCardNumber(user.getId(), cardRedis.getNumber());
                        ...
                        if(findCardRes.isPresent()) {
                            ...
                            //보유 카드 조회
                            List<HoldingCard> holdingCardList = holdingCardRepository.findByUserIdAndCardNumber(user.getId(), cardRedis.getNumber());
                            List<LocalDate> holdingCards = holdingCardRepository.findDatesByUserIdAndCardNumber(user.getId(), cardRedis.getNumber());
                            cardGradeDto.setHoldingCards(holdingCards);
                            //보유하고 있는 경우
                            if(!holdingCardList.isEmpty()){
                                readCardResponse.setHave(true);
